...
CREATE TABLE manga_views (
    ...
    CONSTRAINT fk_manga_views_manga FOREIGN KEY (manga_id) REFERENCES manga (ID) ON DELETE CASCADE
);
...
CREATE TABLE chapter_views (
    ...
    total_views INT NOT NULL DEFAULT 0,
    date_added TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT fk_chapter_views_manga FOREIGN KEY (manga_id) REFERENCES manga (ID) ON DELETE CASCADE,
...
CREATE TABLE bookmark (
    ID INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    last_read_chapter_id INT NOT NULL DEFAULT 1,
    date_added TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    user_ID INT NOT NULL,
    manga_ID INT NOT NULL,
...
LEFT JOIN
    chapter_views cv ON c.manga_ID = cv.manga_id AND c.number = cv.chapter_id
GROUP BY
