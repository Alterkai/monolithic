  ...
  <div class="container">
    <div class="flex gap-4 h-[15rem]">
      ...
      <div>
        ...
        <h1 class="font-bold text-3xl">{{ state.title }}</h1>
      </div>
    </div>
...
if (manga_id) {
  ...
  const { data: mangaData } = await useAsyncData('manga-details', () =>
    $fetch<MangaDetails>(`/api/manga/${manga_id}`)
  );

  if (mangaData.value) {
    state.title = mangaData.value.title;
    state.original_title = mangaData.value.original_title;
    state.description = mangaData.value.description;
    state.author = mangaData.value.author;
    state.ratings = mangaData.value.ratings;
    state.genre = mangaData.value.genre.join(', ');
    state.coverPreview = mangaData.value.cover;
  }
}
